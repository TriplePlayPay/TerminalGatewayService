...
#   ...
# }
PAX_CONNECTIONS: Dict[str, Dict[str, Any]] = {}
    ...
    try:
        ...
        async for msg in ws:
            ...
            if terminal_id in PAX_CONNECTIONS.keys():
                ...
                PAX_CONNECTIONS[terminal_id] = {"websocket": ws, "pending_requests": {}}

            # If it's a response to one of our requests, fulfill the future
            if (
                request_id
                and request_id in PAX_CONNECTIONS[terminal_id]["pending_requests"]
            ):
                logger.info(
                    f"Attempting to resolve: {request_id} request at terminal: {terminal_id}"
                )
                fut = PAX_CONNECTIONS[terminal_id]["pending_requests"].pop(request_id)
                logger.info(
        ...
        logger.exception("Exception in pax websocket operations")
...
async def charge(request: Request) -> JSONResponse:
    ...
    logger.info(
        f"Here are connected terminals: {PAX_CONNECTIONS.keys()}"
    )
    ...
    for attempt in range(1, max_retries + 1):
        if terminal_id in PAX_CONNECTIONS:
            ...
            if attempt < max_retries:
                ...
                logger.error(
                    ...
                    f"Terminal {terminal_id} not found. Current connected terminals: {list(PAX_CONNECTIONS.keys())}"
                )
                logger.error(
